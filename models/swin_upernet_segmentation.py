...
class SwinEncoderForUPerNet(SwinTransformerForSimMIM):
    """
...
class SwinUPerNet(nn.Module):
    def __init__(self, encoder, encoder_stride, num_classes):
        ...
        # Derive the channel dimensions for each stage
        # in_channels_list = [encoder.embed_dim * (2 ** i) for i in range(len(encoder.depths))]
        in_channels_list = [256, 512, 1024, 1024]  # matches features from forward_features()
        self.encoder_stride = encoder_stride
        self.decoder = UPerNet(
            in_channels_list= in_channels_list, #encoder.num_features,  # List of feature dimensions from encoder
            num_classes=num_classes
        )

    def forward(self, x):
        features = self.encoder(x)  # returns [C1, C2, C3, C4]
...
def build_segmentation_model(config, num_classes):
    encoder, encoder_stride = build_encoder(config)
    model = SwinUPerNet(
        encoder=encoder,
