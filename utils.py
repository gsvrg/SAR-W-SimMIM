...
def load_checkpoint(config, model, optimizer, lr_scheduler, logger, amp=None):
...
def load_pretrained(config, model, logger): # revised 2025-08-19
    ...
    # Detect if model wraps encoder
    wrapped_encoder = any(k.startswith("encoder.") for k in model_keys)
    pretrained_has_encoder_prefix = any(k.startswith("encoder.") for k in checkpoint_model.keys())

    logger.info(f"Wrapped encoder in model: {wrapped_encoder}")
    logger.info(f"Checkpoint has 'encoder.' prefix: {pretrained_has_encoder_prefix}")

    # Remap keys if necessary
    if wrapped_encoder and not pretrained_has_encoder_prefix:
        checkpoint_model = {"encoder." + k: v for k, v in checkpoint_model.items()}
        logger.info("Added 'encoder.' prefix to checkpoint keys to match model.")
    elif not wrapped_encoder and pretrained_has_encoder_prefix:
        checkpoint_model = {k.replace("encoder.", ""): v for k, v in checkpoint_model.items()}
        logger.info("Removed 'encoder.' prefix from checkpoint keys to match model.")

    # Apply SWIN / VIT specific remapping
